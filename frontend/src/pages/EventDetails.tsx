...
export default function EventDetails() {
  ...
  const handleUpdate = async () => {
    try {
      await api.patch(`${import.meta.env.VITE_API_URL}/events/${id}`, {
        ...editData,
  ...
  if (!event) return (
    <div className="min-h-screen bg-brand-white dark:bg-brand-darkBg flex justify-center items-center text-gray-500 dark:text-brand-darkText">
      Event not found
    </div>
  );
    ...
    <div className="min-h-screen bg-brand-white dark:bg-brand-darkBg transition-colors">
          ...
          <div className="p-8">
            <div className="flex justify-between items-start mb-6">
              {isEditing ? (
                <div className="flex-1 mr-4">
                  ...
                  <div className="mb-4">
                    ...
                    <TagSelect value={selectedTags} onChange={setSelectedTags} options={availableTags} />
                  </div>
                  <textarea
                  ...
                  <div className="mt-4 flex gap-2">
                    <button onClick={handleUpdate} className="bg-brand-green hover:opacity-90 transition-opacity text-white px-4 py-2 rounded-lg flex items-center gap-2">
                      <Save className="w-4 h-4" /> Save
                    </button>
                    <button onClick={() => {
                      setIsEditing(false);
                      if (event.tags) setSelectedTags(event.tags.map((t: any) => ({ label: t.name, value: t.name })));
                    }} className="bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors text-gray-700 dark:text-gray-300 px-4 py-2 rounded-lg flex items-center gap-2">
                ...
                <div className="flex-1 pr-4">
                  ...
                  {event.tags && event.tags.length > 0 && (
                    <div className="flex flex-wrap gap-2 mb-4">
                      {event.tags.map(tag => <TagChip key={tag.id} name={tag.name} />)}
                    </div>
                  )}
              ...
              {isOrganizer && !isEditing && (
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setShowDeleteModal(true)} className="p-2 text-red-500 bg-red-50 dark:bg-red-500/10 rounded-lg hover:bg-red-100 dark:hover:bg-red-500/20 transition-colors">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50 dark:bg-brand-darkBg rounded-xl p-6 border border-gray-100 dark:border-brand-darkBorder transition-colors">
              ...
              <div className="flex items-center gap-3 text-gray-800 dark:text-gray-200">
                ...
                <span className="font-medium">{event.participants.length} / {event.capacity || 'Unlimited'} participants</span>
              </div>
            </div>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black/50 dark:bg-black/70 flex items-center justify-center z-50 px-4">
          <div className="bg-white dark:bg-brand-darkCard rounded-xl shadow-2xl max-w-md w-full p-6 border dark:border-brand-darkBorder">
            ...
            <div className="flex justify-end gap-3">
              <button onClick={() => setShowDeleteModal(false)} className="px-4 py-2 text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg font-medium transition-colors">
                Cancel
              </button>
              <button onClick={confirmDelete} className="px-4 py-2 text-white bg-red-600 hover:bg-red-700 rounded-lg font-medium shadow-sm transition-colors">
                Delete
              </button>
            </div>
          </div>
